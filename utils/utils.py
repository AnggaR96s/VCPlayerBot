    ...
    if not Config.CALL_STATUS or not Config.IS_ACTIVE:
        if Config.SCHEDULE_LIST[0]['job_id'] == job_id \
            ...
            try:
                ...
                LOGGER.error(
                    "Unable to schedule VideoChat, since date is invalid")
            except Exception as e:
                LOGGER.error(
                    f"Error in scheduling voicechat- {e}",
                    exc_info=True)
    await sync_to_db()
...
async def run_schedule(job_id):
    ...
    if not data:
        LOGGER.error(
            "The Scheduled stream was not played, since data is missing")
        ...
        if Config.HAS_SCHEDULE:
            if not await start_scheduled():
                LOGGER.error(
                    "Scheduled stream skipped, Reason - Unable to start a voice chat.")
                return
        data_ = [{1: data['1'], 2:data['2'],
...
async def skip():
    if Config.STREAM_LINK and len(Config.playlist) == 0 and Config.IS_LOOP:
        ...
        return
            ...
            and Config.IS_LOOP:
        LOGGER.info(
            "Loop Play enabled, switching to STARTUP_STREAM, since playlist is empty.")
        await start_stream()
        return
    elif not Config.playlist \
            and not Config.IS_LOOP:
        LOGGER.info(
            "Loop Play is disabled, leaving call since playlist is empty.")
        await leave_call()
        return
...
async def check_vc():
    ...
    if a.full_chat.call is None:
        try:
            ...
            LOGGER.error(
                f"Unable to start new GroupCall :- {e}",
                exc_info=True)
            return False
    else:
        if Config.HAS_SCHEDULE:
...
async def join_call(link, seek, pic, width, height):
    if not await check_vc():
        LOGGER.error(
            "No voice call found and was unable to create a new one. Exiting...")
        return
    if Config.HAS_SCHEDULE:
        await start_scheduled()
    ...
    try:
        if seek:
            ...
            if not Config.IS_VIDEO:
                ...
                if pic:
                    cwidth, cheight = resize_ratio(
                        1280, 720, Config.CUSTOM_QUALITY)
                    await group_call.join_group_call(
                        int(Config.CHAT),
                        AudioImagePiped(
                            link,
                    ...
                    if not width \
                            or not height:
                        LOGGER.error(
                            "No Valid Video Found and hence removed from playlist.")
                        if Config.playlist or Config.STREAM_LINK:
                            return await skip()
                        else:
                            LOGGER.error(
                                "This stream is not supported , leaving VC.")
                            return
                    cwidth, cheight = resize_ratio(
                        width, height, Config.CUSTOM_QUALITY)
                    await group_call.join_group_call(
                        int(Config.CHAT),
                        AudioVideoPiped(
                            link,
        ...
        LOGGER.error(
            f"Errors Occured while joining, retrying Error- {e}",
            exc_info=True)
        return False
...
def is_ytdl_supported(input_url: str) -> bool:
    ...
    return any(int_extraactor.suitable(input_url)
               and int_extraactor.IE_NAME != "generic" for int_extraactor in shei)
...
async def set_up_startup():
    ...
    if Config.STREAM_URL.startswith(
            "@") or (str(Config.STREAM_URL)).startswith("-100"):
        Config.CPLAY = True
...
async def start_stream():
    ...
    if Config.YPLAY:
        try:
            ...
            LOGGER.error(
                "Unable to fetch youtube playlist.Recheck your startup stream.")
...
async def stream_from_link(link):
    ...
    if not link:
        LOGGER.error(
            "Unable to obtain sufficient information from the given url")
...
async def get_link(file):
    ytdl_cmd = ["yt-dlp", "--geo-bypass", "-g", "-f",
                "best[height<=?720][width<=?1280]/best", file]
...
async def edit_title():
    ...
    try:
        ...
        LOGGER.error(
            f"Errors Occured while editing title - {e}",
            exc_info=True)
...
async def shuffle_playlist():
    v = []
    [v.append(Config.playlist[c]) for c in range(2, len(Config.playlist))]
    random.shuffle(v)
...
async def y_play(playlist):
    try:
        ...
        LOGGER.error(
            f"Errors Occured While Importing Playlist - {e}",
            exc_info=True)
        Config.YSTREAM = True
        Config.YPLAY = False
        if Config.IS_LOOP:
...
async def c_play(channel):
    ...
    try:
            ...
            if filter in Config.FILTERS:
                async for m in USER.search_messages(chat_id=channel, filter=filter):
                    ...
                    if filter == "audio":
                        ...
                        if "video" not in you.document.mime_type:
        ...
        if who == 0:
            LOGGER.warning(
                f"No files found in {chat.title}, Change filter settings if required. Current filters are {Config.FILTERS}")
            if Config.CPLAY:
                Config.CPLAY = False
                Config.STREAM_URL = "https://www.youtube.com/watch?v=zcrUCvBD16k"
                LOGGER.warning(
                    "Seems like cplay is set as STARTUP_STREAM, Since nothing found on {chat.title}, switching to 24 News as startup stream.")
        ...
        LOGGER.error(
            f"Errors occured while fetching songs from given channel - {e}",
            exc_info=True)
        if Config.CPLAY:
            Config.CPLAY = False
            Config.STREAM_URL = "https://www.youtube.com/watch?v=zcrUCvBD16k"
            LOGGER.warning(
                "Seems like cplay is set as STARTUP_STREAM, and errors occured while getting playlist from given chat. Switching to 24 news as default stream.")
            Config.STREAM_SETUP = False
        await sync_to_db()
...
async def volume(volume):
    try:
        ...
        LOGGER.error(
            f"Errors Occured while changing volume Error -{e}",
            exc_info=True)
...
async def get_admins(chat):
    ...
    if not Config.ADMIN_CACHE:
        ...
        try:
            ...
            LOGGER.error(
                f"Errors occured while getting admin list - {e}",
                exc_info=True)
        ...
        for var in ENV_VARS:
            ...
            if prev_default is not None:
                ...
                if current_value != prev_default:
                    LOGGER.info(
                        "ENV change detected, Changing value in database.")
    ...
    try:
        ...
        if not n:
            ...
            if os.path.isfile(file):  # if ts a file, its a tg file
                ...
                while not (os.stat(file).st_size) >= total:
                    LOGGER.info(
                        f"Downloading {Config.playlist[0][1]} - Completed - {round(((int(os.stat(file).st_size)) / int(total))*100)} %")
                    await sleep(5)
...
def resize_ratio(w, h, factor):
    ...
    # https://github.com/pytgcalls/pytgcalls/issues/118
...
async def startup_check():
    if Config.LOG_GROUP:
        try:
            ...
            LOGGER.error(
                f"LOG_GROUP var Found and @{Config.BOT_USERNAME} is not a member of the group.")
    ...
    if Config.RECORDING_DUMP:
        try:
            ...
            LOGGER.error(
                f"RECORDING_DUMP var Found and @{Config.USER_ID} is not a member of the group./ Channel")
        ...
        if k.status not in ["administrator", "creator"]:
            LOGGER.error(
                f"RECORDING_DUMP var Found and @{Config.USER_ID} is not a admin of the group./ Channel")
    ...
    if Config.CHAT:
        try:
            ...
            if k.status not in ["administrator", "creator"]:
                LOGGER.warning(
                    f"{Config.USER_ID} is not an admin in {Config.CHAT}, it is recommended to run the user as admin.")
            elif k.status in ["administrator", "creator"] and not k.can_manage_voice_chats:
                LOGGER.warning(
                    f"{Config.USER_ID} is not having right to manage voicechat, it is recommended to promote with this right.")
        except (ValueError, PeerIdInvalid, ChannelInvalid):
            Config.STARTUP_ERROR = f"The user account by which you generated the SESSION_STRING is not found on CHAT ({Config.CHAT})"
            LOGGER.error(
                f"The user account by which you generated the SESSION_STRING is not found on CHAT ({Config.CHAT})")
            return False
        try:
            k = await bot.get_chat_member(Config.CHAT, Config.BOT_USERNAME)
            if not k.status == "administrator":
                LOGGER.warning(
                    f"{Config.BOT_USERNAME}, is not an admin in {Config.CHAT}, it is recommended to run the bot as admin.")
        except (ValueError, PeerIdInvalid, ChannelInvalid):
            Config.STARTUP_ERROR = f"Bot Was Not Found on CHAT, it is recommended to add {Config.BOT_USERNAME} to {Config.CHAT}"
            LOGGER.warning(
                f"Bot Was Not Found on CHAT, it is recommended to add {Config.BOT_USERNAME} to {Config.CHAT}")
    if not Config.DATABASE_URI:
        LOGGER.warning(
            "No DATABASE_URI , found. It is recommended to use a database.")
    return True
