...
@Client.on_message(reply_filter & filters.private & ~filters.bot & filters.incoming &
                   ~filters.service & ~filters.me & ~filters.chat([777000, 454000]))
async def reply(client, message):
    try:
        ...
        Config.msg[message.chat.id] = {
            "msg": m.updates[1].message.id, "s": message.message_id}
...
async def handle_raw_updates(client: Client, update: Update, user: dict, chat: dict):
    if isinstance(update, UpdateGroupCallParticipants):
        ...
        if Config.CURRENT_CALL and update.call.id == Config.CURRENT_CALL:
            ...
            old = list(
                filter(
                    lambda k: k.peer.user_id if hasattr(
                        k.peer,
                        'user_id') else k.peer.channel_id == Config.USER_ID,
                    all))
            if old:
                for me in old:
                    if me.volume:
    ...
    if isinstance(update, UpdateGroupCall) and (
            update.chat_id == int(-1000000000000 - Config.CHAT)):
...
async def handler(client: PyTgCalls, update: Update):
    if isinstance(
            update,
            StreamAudioEnded) or isinstance(
            update,
            StreamVideoEnded):
