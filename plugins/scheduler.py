...
@Client.on_message(filters.command(
    ["schedule", f"schedule@{Config.BOT_USERNAME}"]) & chat_filter & admin_filter)
...
@Client.on_message(filters.command(["slist",
                                    f"slist@{Config.BOT_USERNAME}"]) & admin_filter & chat_filter)
async def list_schedule(bot, message):
    ...
    for sch in s:
        ...
        date_ = (
            (pytz.utc.localize(
                date, is_dst=None).astimezone(IST)).replace(
                tzinfo=None)).strftime("%b %d %Y, %I:%M %p")
...
@Client.on_message(filters.command(
    ["cancel", f"cancel@{Config.BOT_USERNAME}"]) & admin_filter & chat_filter)
...
@Client.on_message(filters.command(
    ["cancelall", f"cancelall@{Config.BOT_USERNAME}"]) & admin_filter & chat_filter)
