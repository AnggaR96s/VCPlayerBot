    ...
    with suppress(MessageIdInvalid, MessageNotModified, QueryIdInvalid):
        ...
        if query.data.startswith("sch"):
            ...
            if data.startswith("sch_month"):
                ...
                if yea_r == "choose":
                    ...
                    for month in months:
                        ...
                        if k < int(today.month):
                            year_ += 1
                            button_.append([InlineKeyboardButton(
                                text=f"{str(month)}  {str(year_)}", callback_data=f"sch_showdate_{year_}_{k}")])
                        else:
                    ...
                    for i in splited:
                        ...
                        for d in i:
                            k.append(
                                InlineKeyboardButton(
                                    text=f"{d}",
                                    callback_data=f"sch_day_{year}_{month}_{date}_{d}"))
                        button.append(k)
                    if month == today.month and date < today.day and year == today.year + 1:
                        pyear = year - 1
                    else:
                        pyear = year
                    button.append([InlineKeyboardButton("Back",
                                                        callback_data=f"sch_showdate_{pyear}_{month}"),
                                   InlineKeyboardButton("Close",
                                                        callback_data="schclose")])
                ...
                for i in range(0, len(l), 6):
                    ...
                    for d in chunk:
                        k.append(
                            InlineKeyboardButton(
                                text=f"{d}",
                                callback_data=f"sch_minute_{year}_{month}_{day}_{hour}_{d}"))
                    button.append(k)
                button.append([InlineKeyboardButton("Back",
                ...
                button = [
                    [
                        InlineKeyboardButton(
                            "Confirm",
                            callback_data=f"schconfirm_{year}-{month}-{day} {hour}:{minute}"),
                        InlineKeyboardButton(
                            "Back",
                ...
                button = []
                button.append([InlineKeyboardButton(
                    text=f"{str(thissmonth)}  {str(year)}", callback_data=f"sch_month_choose_none_none")])
                ...
                Config.SCHEDULE_LIST = sorted(
                    Config.SCHEDULE_LIST, key=lambda k: k['date'])
            ...
            # if not (Config.playlist or Config.STREAM_LINK):
                # return await query.answer("Startup stream cant be seeked.",
                # show_alert=True)
